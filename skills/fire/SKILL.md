...
If `codex` is missing, unauthenticated, or the profile doesn't exist (preflight step 2), say so and offer to run `/expo:mise` - don't silently implement the task yourself without telling the user the delegation failed.
