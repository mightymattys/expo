...
Ways to route a fired ticket to a worker other than the default Codex model:
two GLM-5.2 routes, and a no-extra-key Claude route (Sonnet 5). `/expo:mise`
...
marker: `~/.expo/glm-claude/settings.json` exists. Requires `ZAI_API_KEY` in the
environment.

```
Bash (run_in_background: true), cwd = repo root:
env CLAUDE_CONFIG_DIR="$HOME/.expo/glm-claude" \
...
Installed marker: `~/.codex/expo-glm.config.toml` exists (the file is
...
```
Bash (run_in_background: true), cwd = repo root:
codex exec --profile expo-glm \
  --output-last-message "$JOB/result.md" \
  - < "$JOB/ticket.md" > "$JOB/job.log" 2>&1
```

Preflight for this route: `test -f ~/.codex/expo-glm.config.toml` - Codex
...
- Cross-model review: when Codex quota recovers, keep Codex as the taster -
  Sonnet implements, the Codex model reviews, the head chef orchestrates. If both
...
- No ledger line - `claude -p` emits no token summary (same gap as
  Route A).
- Invocation spelling: `/expo:fire --with sonnet <task>` (or the loose
