...
If `codex` is missing or `~/.codex/expo.config.toml` doesn't exist, stop and offer `/expo:mise` (Codex silently ignores a missing profile - check with `test -f`, don't rely on an error).
...
- Post-fire review (a pre-fire baseline exists - inside `/expo:serve`, or tasting
...
Mint a job dir (`JOB=$(mktemp -d "$SCRATCHPAD/taste-<label>-XXXXXX")`, where `$SCRATCHPAD` is your session scratchpad directory - substitute its absolute path); a short descriptive label is welcome. Stamp its start (`date -u +%Y-%m-%dT%H:%M:%SZ > "$JOB/started"` - the ledger's `claude_tokens` window). Before running the reviewer, materialise the exact reviewed diff as `$JOB/scope.patch`: the post-fire delta against the baseline when one exists; otherwise the working tree, including untracked files via the temporary-index pattern used by the tree anchor below. Build the review prompt from [references/review-prompt.md](references/review-prompt.md), point its Scope at the absolute `$JOB/scope.patch`, and add any focus the user gave, then:
...
- If OpenAI's official `codex` plugin is installed, `/codex:review` also exists. `/expo:taste` differs on purpose: pinned read-only sandbox, scale-calibrated prompt, and the mandatory validation pass that filters false positives.
...
- Inside `/expo:serve`, taste runs as a pipeline stage the user ordered up front - deliberate, budgeted scope, not an unbounded stop-time or pre-commit review gate.
