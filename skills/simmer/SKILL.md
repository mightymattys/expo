...
If `codex` is missing or `~/.codex/expo.config.toml` doesn't exist, stop and
offer `/expo:mise` first (Codex silently ignores a missing profile - `test -f`).
...
If `.expo/loop.md` already exists at invocation, this is a resume: on a task
...
4. **Judge, decide, and say so** - give the user a one-line lap report (lap N of M:
   ...
   same `~/.expo/ledger.jsonl` line with `"skill":"simmer"` plus `"lap":N`, pass
   ...
   - Checks pass → done. Report laps used, final check output, the commits made, and
     **the branch name** - merging (or deleting) it is the user's call. Mention that
     `.expo/loop.md` and `progress.md` are loop scaffolding they can drop, while
...
- For recurring maintenance loops (babysit CI, rebase branches, flaky-test repair),
  ...
  a fresh session finds `.expo/loop.md` and resumes at the recorded lap. A cloud
