## Division of labor (expo, autonomous routing)
...
- Route by task shape, not by whether a slash command was typed. Invoke the skill via
  ...
  - Spec-able implementation with checkable done criteria - multi-file features,
    mechanical refactors, migrations, bulk boilerplate - goes to expo:serve end
    to end; use expo:fire instead when the work should pause for review between
    stages.
  - A request for a review, a second opinion, or a cross-model check goes to
    expo:taste.
  - "Fix the findings" after a taste goes to expo:refire.
