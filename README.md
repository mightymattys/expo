...
**`/expo:serve`** is for task-shaped work, done end to end: implement,
...
**`/expo:simmer`** is for goal-shaped work, looped until a command passes:
...
| `/expo:mise` | Setup: Codex CLI + auth checks, delegation profile, `AGENTS.md` scaffold, routing policy (manual or autonomous). Once per machine, once per repo. |
| `/expo:receipts` | Print the check: the repo's last ten run receipts as a table with a savings total; reprint any receipt's shareable summary. |
...
(`expo@expo` is `plugin@marketplace` - same name for both here.) Then,
inside a repo:
...
```text
you ── "/expo:serve migrate auth" ──▶ CLAUDE (head chef)
                                              │ ticket: files ±, done-when,
                                              │ verification commands
                                              ▼
            ┌────────────────────────────────────────────┐
            │ codex exec --profile expo                  │  background;
...
why expo uses a thin wrapper instead of a persistent MCP server.
...
`/plugin uninstall expo` removes the skills (and
...
- a `.expo/` directory (run receipts, and loop state after a simmer) in repos
  where a serve or simmer ran - git-ignored via a `.expo/` line in
  `.git/info/exclude`; `rm -rf .expo` per repo when you're done with the
  receipts
